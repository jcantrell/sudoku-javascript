...
class LogicSudokuSolver {
  ...
  solveBoard() {
    ...
    // Step 2: Step through each possibility, checking if it produces a solution
    for (var i=1;i<512;i<<=1) {
      if ((i&this.PossibilityGrid[minx][miny]) != 0) {
        //if (this.checkGuess(minx,miny,this.bit2num(i)))
        if (this.checkGuess(minx,miny,i))
          return true;
				else
					this.removePossibility(i,minx,miny);
      }
    }
    return false;
  }
